...
import org.apache.http.client.methods.{HttpPost => ApacheHttpPost}
...
trait HttpClient {
  ...
  def httpPost(uri: String, header: String, data: Array[Byte]): ApacheHttpPost
}

class HttpClientUsingCertificates(certFilePath: String, certKeyPath: String) extends HttpClient {
  ...
  override def httpPost(uri: String, header: String, data: Array[Byte]): ApacheHttpPost = {
    val p = new ApacheHttpPost(uri)
  ...
  private def sslContext = {
    ...
    ctx.init(keyManager, Array[TrustManager] {trustManager}, _)
    ctx
  }
  ...
  private def certificate = {
    ...
    new BufferedInputStream(new FileInputStream(certFilePath)) { bis =>
  ...
  private def key = {
    ...
    new FileInputStream(certKeyPath) { fis =>
